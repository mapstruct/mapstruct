...
In the case that the `Fruit` is an abstract class or an interface, you would get an `IllegalArgumentException` because it is unknown how to map a `GrapeDto`.
...
If you use an update method with a return value then the subclass mappings will return a new instance based on the mapping, but it will not take into account the `@MappingTarget` parameter.
