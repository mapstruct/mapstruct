...
MapStruct also has a mechanism for mapping any remaining (unspecified) mappings to a default. This can be used only once in a set of value mappings and only applies to the source. It comes in two flavors: `<ANY_REMAINING>` and `<ANY_UNMAPPED>`. The can not be used at the same time.
...
MapStruct supports enum to a String mapping along the same lines as is described from enum-to-enum type above. There are similarities and differences:
...
2. Similarity: `<ALL_UNMAPPED`> stops after handling defined mapping and proceeds to the default clause value.
3. Difference: `<ALL_REMAINING>` will result in an error. It acts on the premise that there is name similarity between enum constants in source and target which does not make sense for a String type.
...
2. Similarity: `<ALL_UNMAPPED`> stops after handling defined mapping and proceeds to the default clause value.
3. Difference: `<ALL_REMAINING>` will result in an error (same reasoning enum-to-`String`.
4. Difference: Given 1. and 3. there will never be unmapped values.
