...
Sometimes mappings are not straightforward, and some fields should be mapped using plain java code, while mapping the others of them is covered by generated method.

Let's see if there are properties `length`, `width` and `height` in `FishTank`, and `VolumeDto` inner object, which contains `volume` and `description` fields in `FishTankWithVolumeDto`.
We could convert the remainder of the fields using mappings defined defined by means of @Mapping annotations, however it requires implementing method to create `VolumeDto`.
...
Note that there is `@Mapping` annotation where `source` parameter equied to `"source"`, it should be same as parameter name in `map(FishTank source)` method.
