...
Sometimes mappings are not straightforward, and some fields should be mapped using plain java code, while mapping latter of them is covered by generated method.

Let's see if there is a properties `length`, `width` and `height` in `FishTank`, and `VolumeDto` inner object, which contains `volume` and `description` fields in `FishTankWithVolumeDto`.
We could convert rest of the fields using mappings defined inside `@Mapper` annotations, however it requires implementing method to create `VolumeDto`.
This could be done by just creating a method which takes `FishTank` instance as a parameter, and returns a `VolumeDto`.
...
@Mapper
public class FishTankMapperWithVolume {
    ...
    VolumeDto mapVolume(FishTank source) {
        int volume = soruce.length * source.width * source.height;
...
Note that no `@Mapping` annotation is needed, mapper will use `mapVolume` while calling `map` method.
