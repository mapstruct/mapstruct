...
[[fluent-access-mappings]]
=== Mappings with fluent vs JavaBean access

* MapStruct does not support fluent getters, like `String name()`. Getters of classes which are read by MapStruct mapping must be named with 'String getName()'.
* MapStruct does support fluent setters in both builders and non-immutable objects, like `Builder name(String value)` and `Person name(String value)`.

See https://github.com/arberg/mapstruct-examples/blob/master/mapstruct-on-android-with-autovalue-builders/app/src/main/java/com/example/mapstructtest/model/CarFluentSetter.java[example with and without builder] and Person example in the `Using buildesrs` section.
...
If a Builder exists for a certain type, than that builder will be used for the mappings. It is not necesarry to supply a `BuilderProvider`,
...
public class Person {
    ...
    // The getter of style getX is required only if mapping from this object, not if mapping to this object
...
When using FreeBuilder then the JavaBean convention should be followed with `setName/getName`, as MapStruct does not support fluent getter access `String name()`.
...
Note MapStruct 1.3 does NOT support immutable objects that are created via Constructor. It may arrive in future versions.

See our https://github.com/mapstruct/mapstruct-examples/mapstruct-on-android-with-autovalue-builders[MapStruct Examples] for a complete example with builders.
