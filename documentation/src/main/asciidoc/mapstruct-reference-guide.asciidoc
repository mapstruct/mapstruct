...
MapStruct offers the possibility to override the `AccessorNamingStrategy` via the SPI interface. A nice example is the use of the fluent API on the source object `GolfPlayer` and `GolfPlayerDto` below.
...
This can be achieved with implementing a ServiceProviderInterface (SPI). Here's an implemented `org.mapstruct.ap.spi.AccessorNamingStrategy`:
...
----
====
Adding an SPI is as simple as adding a file named `org.mapstruct.ap.spi.AccessorNamingStrategy` with the simple content `org.mapstruct.example.CustomAccessorNamingStrategy` (FQN) to the `META-INF/services/` folder.

This as a rudimentary SPI which of course can be improved. But the uptake is that MapStruct identifies a target write accessor (setter) as starting with "with" and a target read accessor with no particular prefix. The generated code will use that 'setter' and 'getter' definition.
...
----
    @Override
    public GolfPlayerDto golfPlayerToDto(GolfPlayer player) {
        if ( player == null ) {
            return null;
        }
        ...
        golfPlayerDto.withHandicap( player.handicap() );
        golfPlayerDto.withName( player.name() );
