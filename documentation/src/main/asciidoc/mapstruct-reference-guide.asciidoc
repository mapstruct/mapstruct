...
If you don't want explicitly name all properties from nested source bean, you can use `.*` notation in source property. This will tell MapStruct to map every property from source bean to same property on target object. The following shows an example:
