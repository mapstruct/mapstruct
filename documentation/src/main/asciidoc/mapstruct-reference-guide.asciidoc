...
[TIP]
====
Suppose the signature was: `DeliveryAddressDto personAndAddressToDeliveryAddressDto(Person person, Integer hn)`.
Then by referring to the method parameter `hn` instead of a property, a non bean type (like the `java.lang.Integer`) can be mapped directly to a target property:
