...
In case a component model was specified, but the mapper must be instantiated using the default constructor the `usesPlain` attribute could be used like in the following example.

.Referencing another mapper class without injecting it
====
[source, java, linenums]
[subs="verbatim,attributes"]
----
@Mapper(usesPlain = DateMapper.class, componentModel = "cdi")
public class CarMapper {
...
When having a custom mapper hooked into the generated mapper with `@Mapper#uses()` or `@Mapper#usesPlain()`, an additional parameter of type `Class` (or a super-type of it) can be defined in the custom mapping method in order to perform general mapping tasks for specific target object types. That attribute must be annotated with `@TargetType` for MapStruct to generate calls that pass the `Class` instance representing the corresponding property type of the target bean.
...
Callback methods can be implemented in the abstract mapper itself, in a type reference in `@Mapper#uses()` or `@Mapper#usesPlain()`, or in a type used as `@Context` parameter.
...
4. Methods from types referenced in `Mapper#usesPlain()`, in the order of the type declaration in the annotation.
5. Methods declared in one type are used after methods declared in their super-type.
