...
The default implementation of the `BuilderProvider` requires the following for a type to have a builder:

* The type (`Person`) has a parameterless public static method (builder creation method) that returns a type (`PersonBuilder`) different than the one that needs building
* The found builder type (`PersonBuilder`) has a parameterless public method (build method) that returns the type being build (`Person`)
...
If no factory method exists for instantiating the builder type the builder creation method would be used.
To finish the mapping MapStruct will invoke the build method of the builder.
