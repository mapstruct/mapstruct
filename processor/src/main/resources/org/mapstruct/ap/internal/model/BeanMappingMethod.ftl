...
<#lt>${accessibility.keyword} <@includeModel object=returnType/> ${name}(<#list parameters as param><@includeModel object=param/><#if param_has_next>, </#if></#list>)<@throws/> {
    ...
    <#if hasSubclassMappings()>
        <#if subclassMappings[0].sourceArgumentType.isSealed()>
            <#assign switchVar = subclassMappings[0].sourceArgument>
            switch (${switchVar}) {
            <#list subclassMappings as subclass>
                case <@includeModel object=subclass.sourceType/> ${subclass.sourceType.name?uncap_first} -> {
    ...
    <#if (!hasSubclassMappings() || !subclassMappings[0].sourceArgumentType.sealed)>
    ...
    <#if hasSubclassMappings() && !subclassMappings[0].sourceArgumentType.isSealed()>
        }
    </#if>
}
<#macro throws>
