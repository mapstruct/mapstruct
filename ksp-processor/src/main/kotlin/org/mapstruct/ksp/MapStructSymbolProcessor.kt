...
class MapStructSymbolProcessor(
    ...
    private fun processMapper(mapper: KSClassDeclaration, resolver: Resolver) {
        ...
        var model: Any? = null
        val processors: List<ModelElementProcessor<Any, Any>> = loadProcessors() as List<ModelElementProcessor<Any, Any>>
        for (processor in processors) {
            try {
                logger.info("Executing processor: ${processor.javaClass.simpleName}")
                model = processor.process(processorContext, mapperTypeElement, model)
     ...
     */
    private fun loadProcessors(): List<ModelElementProcessor<*, *>> {
        val serviceLoader = ServiceLoader.load(
            ModelElementProcessor::class.java,
            ModelElementProcessor::class.java.classLoader
        )
